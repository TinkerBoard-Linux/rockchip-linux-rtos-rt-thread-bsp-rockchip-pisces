...
static void olpc_ap_thread(void *p)
{
    ...
    while (1)
    {
        ...
        case FIRMWARE_INFO_REQ:
            ret = olpc_ap_mbox_send(FIRMWARE_INFO_REQ, mq.msg, sizeof(FIRMWARE_REQ_PARAM));
            if (ret == RT_EOK)
            {
                olpc_firmware_info_show(mq.msg);
        ...
        case FIRMWARE_DOWNLOAD_REQ:
            ret = olpc_ap_mbox_send(FIRMWARE_DOWNLOAD_REQ, mq.msg, sizeof(FIRMWARE_REQ_PARAM));
            if (ret == RT_EOK)
            {
                olpc_firmware_cache_invalidate(mq.msg);
                rt_event_send(ap_ack_event, AP_ACK_OK);
            }
            else
            {
